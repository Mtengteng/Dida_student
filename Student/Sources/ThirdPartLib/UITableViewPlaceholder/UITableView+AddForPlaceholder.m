...
- (void)layoutDefaultView:(UIView *)defaultView {
    ...
    UIImage *image = self.defaultNoDataImage ? : [UIImage imageNamed:@"new_noVob.png"];
    imageView.image = image;
    CGFloat X = (SCREEN_WIDTH - image.size.width - self.contentInset.left - self.contentInset.right) / 2;
    CGFloat Y = (SCREEN_HEIGHT - image.size.height - self.contentInset.top - self.contentInset.bottom) / 3 - 20;
    ...
    label.frame = CGRectMake(0, imageView.frame.origin.y + imageView.bounds.size.height + 10, SCREEN_WIDTH, 30);
    label.textColor = BWColor(185.0, 185.0, 185.0, 1.0);
}
...
- (UILabel *)defaultNoDataNoticeLabel {
    NSString *className = NSStringFromClass([self.dataSource class]);
    
    if([className isEqualToString:@"BJLChatViewController"]){
        return nil;
    }else{
        ...
        if (!label) {
            ...
            label.text = self.defaultNoDataText ? : @"对不起，没有符合条件的内容";
...
- (UIImageView *)defaultNoDataNoticeImageView {
    ...
    if([className isEqualToString:@"BJLChatViewController"]){
        ...
        if (!imageView) {
            ...
            imageView.image = self.defaultNoDataImage ? : [UIImage imageNamed:@"new_noVob.png"];
